...
function get_cat() {
    ...
    //назначение новой переменной типа пустой массив
	$arr_cat = array();
	if(mysql_num_rows($result)!=0) {
		//в цикле формируем новый массив
...
//заполнение падающего списка. рекурсия
function fill_select($arr, $parent_id = 0){
	for ($i = 0; $i< count($arr[$parent_id]); $i++){
		
		echo "<option value=\"". $arr[$parent_id][$i]['id'] . "\">".$arr[$parent_id][$i]['title']."</option>";
